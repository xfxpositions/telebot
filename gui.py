...
import threading
from utils import list_audio_devices, init_deepgram, init_stream_audio, init_live_transcription
class App(ctk.CTk):
    def __init__(self):
        super().__init__()
        ...
        self.transcription_values = []
       
        self.started = False
        self.audio_stream_port = 5779
        ...
        self.transcription_textbox = ctk.CTkTextbox(self.transcription_menu, height=200, width=400, corner_radius=10)
        ...
        self.search_textbox = ctk.CTkTextbox(self.search_menu, height=200, width=400, corner_radius=10)
    ...
    def start_button_handler(self):
        if self.started == True:
            # Close 
            self.toggle_button.configure(text='Başlat')  # Change 
            
            
        print("thread starting")
        
        # device_index = self.selected_device_index  # Örnek olarak 0 kullanılmıştır, gerçek uygulamada kullanıcıdan alınmalıdır
        # self.stream_url = init_stream_audio(port=5001, device_index=device_index)
        self.stream_url = "http://localhost:5001/audio"
        
        self.transcription_thread = threading.Thread(target=self.transcription_thread_function, daemon=True)
        self.transcription_thread.start()
        print("thread started")
    def transcription_thread_function(self):
        # Bu fonksiyon, transkripsiyon işlemini asenkron olarak yönetir
        print(f"stream url: {self.stream_url}")
        print("starting")

        init_live_transcription(self.deepgram, stream_url=self.stream_url, language="tr", transcription_textbox=self.transcription_textbox)
